import '../styles/styles.css'; // Pastikan path ini sesuai
...
async function archiveNoteById(id) {
    loadingSpinner.show(); // Show loading spinner
    try {
        const response = await fetch(`${API_URL}/${id}/archive`, { method: 'POST' });
...
async function unarchiveNoteById(id) {
    loadingSpinner.show(); // Show loading spinner
    try {
        const response = await fetch(`${API_URL}/${id}/unarchive`, { method: 'POST' });
...
document.getElementById('viewArchivedButton').addEventListener('click', async function() {
